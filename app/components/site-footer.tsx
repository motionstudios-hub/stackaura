  ...
  return (
    <footer className="relative z-10 border-t border-white/35 bg-white/18 backdrop-blur-2xl">
      <FooterReveal className="mx-auto max-w-[1440px] px-6 py-8 lg:px-10 lg:py-10">
        <div className="overflow-hidden rounded-[34px] border border-white/46 bg-[linear-gradient(180deg,rgba(255,255,255,0.32),rgba(240,247,250,0.22))] shadow-[0_18px_38px_rgba(122,146,168,0.12),inset_0_1px_0_rgba(255,255,255,0.56)] backdrop-blur-2xl">
            ...
            <div className="max-w-xl">
              <Link href="/" className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="truncate text-xl font-semibold tracking-tight text-[#0a2540]">
                    Stackaura
                  </div>
              ...
              <p className="mt-5 text-sm leading-7 text-[#425466] sm:text-[15px]">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {brandPills.map((pill) => (
                  <span
                    key={pill}
                    className="inline-flex items-center rounded-full border border-white/45 bg-white/24 px-3 py-1.5 text-[11px] font-medium uppercase tracking-[0.16em] text-[#425466] shadow-[0_8px_20px_rgba(133,156,180,0.08)] backdrop-blur-2xl"
                  >
                    {pill}
                  </span>
                ))}
              ...
              <div className="mt-6 rounded-[24px] border border-white/42 bg-white/22 p-4 shadow-[0_10px_24px_rgba(133,156,180,0.08)] backdrop-blur-2xl">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {footerSections.map((section) => (
                <div key={section.title}>
                  <h3 className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[#6b7c93]">
                    {section.title}
                  </h3>
                  <ul className="mt-4 space-y-3">
                    {section.links.map((item) => (
                      <li key={`${section.title}-${item.label}`}>
                        <Link
                          href={item.href}
                          className="text-sm leading-6 text-[#425466] transition hover:text-[#0a2540]"
                        >
                          {item.label}
                        </Link>
                      </li>
          ...
          <div className="border-t border-white/38 bg-white/16 px-6 py-4 sm:px-8 lg:px-10">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm text-[#425466]">
                <span className="rounded-full border border-white/40 bg-white/20 px-3 py-1 shadow-[0_8px_18px_rgba(133,156,180,0.08)] backdrop-blur-2xl">
                  South Africa
                </span>
                {legalLinks.map((item) => (
                  <Link
                    ...
                    href={item.href}
                    className={cn(
                      "transition hover:text-[#0a2540]",
                      item.label === "Contact" && "font-medium text-[#0a2540]",
                    )}
                  >
                    {item.label}
                  </Link>
